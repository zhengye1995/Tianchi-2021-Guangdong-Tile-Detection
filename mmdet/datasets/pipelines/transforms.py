...
@PIPELINES.register_module()
class ResizeWithTemplate(object):
    """Resize images & bbox & mask.
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            results['keep_ratio'] = self.keep_ratio
        if self.keep_ratio:
            template_img, scale_factor = mmcv.imrescale(
                results['template_img'],
                results['scale'],
                return_scale=True,
                backend=self.backend)
...
@PIPELINES.register_module()
class RandomFlipWithTemplate(object):
    """Flip the image & bbox & mask.
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                results[key] = mmcv.imflip(
                    results[key], direction=results['flip_direction'])
            results['template_img'] = mmcv.imflip(
                    results['template_img'], direction=results['flip_direction'])
            # flip bboxes
            for key in results.get('bbox_fields', []):
                results[key] = self.bbox_flip(results[key],
                                              results['img_shape'],
...
@PIPELINES.register_module()
class NormalizeWithTemplate(object):
    """Normalize the image.
...
class PadWithTemplate(object):
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            results[key] = padded_img
        if self.size is not None:
            template_padded_img = mmcv.impad(
                results['template_img'], shape=self.size, pad_val=self.pad_val)
        elif self.size_divisor is not None:
            template_padded_img = mmcv.impad_to_multiple(
                results['template_img'], self.size_divisor, pad_val=self.pad_val)
        results['template_img'] = template_padded_img
        results['pad_shape'] = padded_img.shape
        results['pad_fixed_size'] = self.size
        results['pad_size_divisor'] = self.size_divisor
